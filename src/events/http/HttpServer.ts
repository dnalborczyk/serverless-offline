...
export default class HttpServer {
  ...
  private _extractAuthFunctionName(endpoint) {
    const result = authFunctionNameExtractor(endpoint)

    return result.unsupportedAuth ? null : result.authorizerName
  }

  private _configureAuthorization(
    endpoint,
    functionKey: string,
    method,
    path: string,
  ...
  // Bad news
  private _reply500(response, message: string, error) {
  ...
  private _getArrayStackTrace(stack) {
